...
const CountryTab = () => {
    ...
    const [newCountry, setNewCountry] = useState<Omit<Country, "_id">>({
        name: "",
        code: "",
        slug: "",
    });
    ...
    useEffect(() => {
        setNewCountry((g) => ({ ...g, slug: slugify(g.name) }));
    }, [newCountry.name]);
    ...
    const handleDelete = async (id: string) => {
        if (window.confirm("Are you sure you want to delete this Country?")) {
            try {
                ...
                if (response.status === "success") {
                    setToast({ show: true, message: "Country deleted", type: "success" });
                    await fetchCountries();
                } else {
                    setToast({ show: true, message: "Failed to delete Country", type: "error" });
                }
            } catch (error) {
                setToast({ show: true, message: "Failed to delete Country", type: "error" });
            } finally {
    ...
    const handleSave = async () => {
        if (!newCountry.name || !newCountry.code || !newCountry.slug) {
        ...
        try {
            ...
            let response;
            if (editingId) {
                response = await CountryService.updateCountry(editingId, newCountry);
            } else {
                response = await CountryService.createCountry(newCountry);
            }

            if (response.status === "success") {
                setToast({
                    show: true,
                    message: editingId ? "Country updated" : "Country created",
                    type: "success",
                });
                await fetchCountries();
                setEditingId(null);
                ...
                setToast({
                    show: true,
                    message: response.msg || (editingId ? "Failed to update Country" : "Failed to create Country"),
                    type: "error"
                });
            }
        } catch (error) {
            setToast({
                show: true,
                message: editingId ? "Failed to update Country" : "Failed to create Country",
                type: "error",
            });
        } finally {
            setIsSaving(false);
